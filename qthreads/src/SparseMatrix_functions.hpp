...
//row_coefs[loc-row_indices] += input_coefs[i];
	qthread_dincr(&row_coefs[loc - row_indices], input_coefs[i]);
    }
  }
}
...
typedef struct {
       	const MINIFE_LOCAL_ORDINAL*  rowoffsets;
       	const MINIFE_GLOBAL_ORDINAL* cols;
       	const MINIFE_SCALAR*         coefs;
       	const MINIFE_SCALAR*         x;
              MINIFE_SCALAR*         y;
} matvec_std_thread;

void matvec_std_thread_func(size_t start, size_t stop, void* thr_args) {
       	matvec_std_thread* thread_args = (matvec_std_thread*) thr_args;

       	const MINIFE_LOCAL_ORDINAL*  rowoffsets = thread_args->rowoffsets;
       	const MINIFE_GLOBAL_ORDINAL* cols = thread_args->cols;
        const MINIFE_SCALAR*         coefs = thread_args->coefs;
       	const MINIFE_SCALAR*         x = thread_args->x;
              MINIFE_SCALAR*         y = thread_args->y;

        for(size_t row = start; row < stop; row++) {
               	MINIFE_SCALAR sum = 0;

		const MINIFE_LOCAL_ORDINAL start = rowoffsets[row];
		const MINIFE_LOCAL_ORDINAL end   = rowoffsets[row+1];

		#pragma loop_count(15)
               	for(MINIFE_LOCAL_ORDINAL i = start; i < end; i++) {
                        sum += coefs[i] * x[cols[i]];
               	}
            ...
            VectorType& y)
{
  ...
  matvec_std_thread thread_args = { Arowoffsets, Acols, Acoefs, xcoefs, ycoefs };

  // Pass control to qthreads
  QLOOP( 0, n, matvec_std_thread_func, &thread_args );
}
};
