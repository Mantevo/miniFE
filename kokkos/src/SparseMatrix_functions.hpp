...
void init_matrix(MatrixType& M,
                 const Kokkos::vector<typename MatrixType::GlobalOrdinalType>& rows,
                 const Kokkos::vector<typename MatrixType::LocalOrdinalType>& row_offsets,
                 const Kokkos::vector<int>& row_coords,
                 int global_nodes_x,
                 int global_nodes_y,
                 int global_nodes_z,
...
//std::cout<<"  ("<<*loc<<":"<<row_coefs[loc-row_indices]<<" += "<<input_coefs[i]<<")"<<std::endl;
    	Kokkos::atomic_fetch_add(&row_coefs[loc-row_indices], input_coefs[i]);
    }
  }
}
...
void
sum_into_global_linear_system(const ElemData<GlobalOrdinal,Scalar>& elem_data,
...
template<typename MatrixType, typename VectorType, class Device>
struct impose_dirichlet_functorA {
	  typedef Device                   device_type ;
	  typedef typename MatrixType::ScalarType Scalar;
	  typedef typename MatrixType::GlobalOrdinalType GlobalOrdinal;
	  Scalar _prescribed_value;
	  MatrixType _A;
	  VectorType _b;
      Kokkos::vector<int, Device> _bc_rows;
	  impose_dirichlet_functorA(typename MatrixType::ScalarType prescribed_value,
	                      MatrixType& A,
	                      VectorType& b,
	                      Kokkos::vector<int, Device> bc_rows):
	                      _prescribed_value(prescribed_value),_A(A),_b(b),
	                      _bc_rows(bc_rows)
	                      {}
	  //--------------------------------------------------------------------------
	  ...
	  void operator()( const int i ) const
	  {
		  ...
		  GlobalOrdinal last_local_row  = _A.rows.size()>0 ? _A.rows[_A.rows.size()-1] : -1;
		    ...
		    if (row >= first_local_row && row <= last_local_row) {
		      ...
		      _b.coefs[local_row] = _prescribed_value;
		      zero_row_and_put_1_on_diagonal(_A, row);
		    }
	  }
};

template<typename MatrixType, typename VectorType, class Device>
struct impose_dirichlet_functorB {
	  typedef Device                   device_type ;
	  typedef typename MatrixType::ScalarType Scalar;
	  typedef typename MatrixType::GlobalOrdinalType GlobalOrdinal;
	  Scalar _prescribed_value;
	  ...
	  VectorType _b;
      Kokkos::vector<int, Device> _bc_rows;
	  impose_dirichlet_functorB(typename MatrixType::ScalarType prescribed_value,
	                      MatrixType& A,
	                      VectorType& b,
	                      Kokkos::vector<int, Device> bc_rows):
	                      _prescribed_value(prescribed_value),_A(A),_b(b),
	  ...
	  void operator()( const int i ) const
	  {
		    ...
		    GlobalOrdinal* cols = NULL;
		    Scalar* coefs = NULL;
		    _A.get_row_pointers(row, row_length, cols, coefs);

		    Scalar sum = 0;
		    for(size_t j=0; j<row_length; ++j) {
		      if (_bc_rows.find(cols[j]) != _bc_rows.end()) {
		        sum += coefs[j];
		        coefs[j] = 0;
		      }
		    }

		    _b.coefs[i] -= sum*_prescribed_value;
	}
};
//------------------------------------------------------------------------
template<typename MatrixType,
...
impose_dirichlet(typename MatrixType::ScalarType prescribed_value,
                    ...
                    int global_ny,
                    int global_nz,
                    Kokkos::vector<int, typename MatrixType::HostMirror::device_type> bc_rows)
{
  typedef typename MatrixType::GlobalOrdinalType GlobalOrdinal;
  typedef typename MatrixType::LocalOrdinalType LocalOrdinal;
  typedef typename MatrixType::ScalarType Scalar;
