...
struct MatrixInitOp<miniFE::CSRMatrix<MINIFE_SCALAR,MINIFE_LOCAL_ORDINAL,MINIFE_GLOBAL_ORDINAL> > {
  MatrixInitOp(const Kokkos::vector<MINIFE_GLOBAL_ORDINAL>& rows_vec,
               const Kokkos::vector<MINIFE_LOCAL_ORDINAL>& row_offsets_vec,
               const Kokkos::vector<int>& row_coords_vec,
  ...
  GlobalOrdinalType*  dest_rowoffsets;
