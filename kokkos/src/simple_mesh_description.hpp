...
template<typename GlobalOrdinal, class Device = host_device_type>
...
std::cout<<"z=1 BC, node "<<nodeID<<", ("<<ix<<","<<iy<<","<<z1<<")"<<std::endl;
    ...
    v_bc_rows_0 = Kokkos::vector<GlobalOrdinal,Device>(bc_rows_0.size(),0.0);
    v_bc_rows_1 = Kokkos::vector<GlobalOrdinal,Device>(bc_rows_1.size(),0.0);

    typename std::set<GlobalOrdinal>::const_iterator
      bc_iter = bc_rows_0.begin(), bc_end = bc_rows_0.end();
    for(int count = 0; bc_iter!=bc_end; ++bc_iter, count++) {
      v_bc_rows_0(count) = *bc_iter;
    }

    bc_iter = bc_rows_1.begin(), bc_end = bc_rows_1.end();
    for(int count = 0; bc_iter!=bc_end; ++bc_iter, count++) {
      v_bc_rows_1(count) = *bc_iter;
    }
  ...
  Kokkos::vector<GlobalOrdinal,Device> v_bc_rows_0;
  Kokkos::vector<GlobalOrdinal,Device> v_bc_rows_1;
