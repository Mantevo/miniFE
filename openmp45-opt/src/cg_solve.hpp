         ...
         timer_type* my_cg_times)
{
  ...
  #pragma omp target enter data           \
	map(to:  r_ptr[:r.coefs.size()],  \
		 p_ptr[:p.coefs.size()],  \
		Ap_ptr[:Ap.coefs.size()], \
 		 x_ptr[:x.coefs.size()],  \
		 b_ptr[:b.coefs.size()],  \
		 Arowoffsets[:A.row_offsets.size()], \
		 Acols[:A.packed_cols.size()], \
		 Acoefs[:A.packed_coefs.size()]) 
