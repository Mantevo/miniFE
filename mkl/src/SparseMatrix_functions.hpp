            ...
            VectorType& y)
{
  ...
  int n = A.rows.size();
        LocalOrdinalType* Arowoffsets = &A.row_offsets[0];
        GlobalOrdinalType* Acols      = &A.packed_cols[0];
        ScalarType* Acoefs            = &A.packed_coefs[0];
        ScalarType* xcoefs = &x.coefs[0];
        ScalarType* ycoefs = &y.coefs[0];
  ScalarType beta = 0;
  char transpose = 'n';
...
#if defined(MINIFE_MKL_FLOAT)
  mkl_cspblas_scsrgemv(
#elif defined(MINIFE_MKL_DOUBLE)
  mkl_cspblas_dcsrgemv( 
#else
  #error Unknown MINIFE_SCALAR base type.
#endif
	&transpose,
	&n, 
	Acoefs,
