...
#ifdef HAVE_MPI
  magnitude local_dot = result.get_value(), global_dot = 0;
            ...
            VectorType& y)
{
  ...
  cilk_for(int row=0; row<n; row++) {
   	...
   	const MINIFE_LOCAL_ORDINAL start = Arowoffsets[row];
    	const MINIFE_LOCAL_ORDINAL end   = Arowoffsets[row+1];

	// Could do this with
	// sum = __sec_reduce_add(Acoefs[start:end] * xcoefs[Acols[start:end]]);
	// but is appears to seg-fault in 2013.4 compiler.

	#pragma loop_count(15)
    	for(LocalOrdinalType i = start; i < end; ++i) {
      		sum += Acoefs[i]*xcoefs[Acols[i]];
    	}
